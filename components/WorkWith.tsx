      ...
      <div className="site-container">
        <div className="grid items-center gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
          <div>
            ...
            <h2 className="mt-4 text-4xl font-bold text-white sm:text-5xl">{messages.title}</h2>
            <p className="mt-6 text-lg leading-8 text-slate-300">
              {messages.description}
            </p>
            <div className="mt-8 grid grid-cols-1 gap-x-8 gap-y-4 text-base font-semibold leading-7 text-white sm:grid-cols-3 lg:grid-cols-1">
          ...
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-2xl shadow-black/30">
            <Image
              src="/images/expert-network/home-global-expertise.webp"
              alt={messages.imageAlt}
              width={1536}
              height={1024}
              className="aspect-3/2 w-full object-cover"
              sizes="(min-width: 1024px) 55vw, (min-width: 640px) calc(100vw - 3rem), calc(100vw - 3rem)"
            />
          </div>
        </div>
